...
def test_run_enrich_creates_enrichments(db_session):
    ...
    resp = json.dumps(
        [
            {"id": items[0].id, "summary_zh": "摘要一", "category": "model-release", "entities": ["OpenAI"], "tags": ["LLM"], "importance": 8},
    ...
    assert json.loads(enr[items[0].id].entities) == ["OpenAI"]
    assert enr[items[1].id].category == "product-launch"  # 非法分类兜底
...
def test_run_enrich_skips_already_enriched(db_session):
    ...
    p1 = MockProvider(responses=[json.dumps([{"id": items[0].id, "summary_zh": "x", "category": "research", "tags": [], "importance": 5}])])
